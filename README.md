...
- **Free backup tutor when the budget runs out:** if OpenAI returns a quota error, the studio does not simply fail — it reconnects the same lesson to Google's Gemini Live API and carries on. The fallback runs the identical tool catalog, whiteboard, wake word and saved transcript, but it is one model doing both the talking and the reasoning, so it is slower and weaker at the teaching rules. It announces itself in the lesson panel and never engages silently. Audio is relayed through the app server, so the Gemini key stays server-side exactly as the OpenAI one does. **On Google's free tier the conversation may be used to train their models**, which is why the notice says so and why leaving `GEMINI_API_KEY` empty disables the tier entirely.
...
- **Virgil can see the interface:** a `look_at_screen` tool returns a structured description of the live lesson UI (open section, selected question, the learner's draft, focused control, visible text and controls, whiteboard contents). A change of learning focus sends the full picture to the voice model and a one-line headline to the reasoning backend, and a `navigate_lesson` tool lets Virgil open a section or jump to a practice question.
- **Backend history budget:** the delegated backend keeps at most 128 items and 32768 UTF-8 bytes of input history per session, so everything the browser adds is measured first (`BackendHistoryBudget`). Tool outputs hold a reserve that passive UI notes cannot touch, so a function call can always be answered. Screenshots and whiteboard images are budgeted the same way and in practice stay out of that history — a measured data URL is 22KB at even 384px wide, against a 32768-byte session — so the text description is what the tutor reasons from. When the history does fill, the app says so and the session can be saved and reconnected.
